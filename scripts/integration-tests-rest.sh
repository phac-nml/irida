...
./install-libs.sh
popd
xvfb-run -a mvn clean verify -Prest_testing -Dliquibase.config.port=3308 -Djetty.port=8082 -Dtest.galaxy.database.connection="mysql://test:test@localhost/irida_galaxy_test?unix_socket=/var/run/mysqld/mysqld.sock"
