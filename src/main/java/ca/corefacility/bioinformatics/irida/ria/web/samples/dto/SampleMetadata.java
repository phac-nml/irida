...
public class SampleMetadata {
    ...
    private List<SampleMetadataFieldEntry> getMapForEntries(Set<MetadataEntry> metadataEntries) {
        List<SampleMetadataFieldEntry> metadata = metadataEntries.stream().map(s -> new SampleMetadataFieldEntry(s.getField().getId(), s.getField().getLabel(), s.getValue(), s.getId())).collect(Collectors.toList());
