...
public class UIPipelineStartService {
	...
	public void start(UUID id, LaunchRequest request) throws IridaWorkflowNotFoundException {
		...
		NAMED PARAMETERS
		 */
		IridaWorkflowNamedParameters namedParameters;
		...
		if (inputs.requiresSingleSample()) {
			submissionService.createSingleSampleSubmission(workflow, request.getReference(), singles, pairs,
					request.getParameters(), null, request.getName(), request.getDescription(), null,
					request.isUpdateSamples(), request.isEmailPipelineResult());
		}
