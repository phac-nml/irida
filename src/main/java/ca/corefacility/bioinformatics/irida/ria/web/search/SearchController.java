...
public class SearchController {
	...
	public ResponseEntity<AntTableResponse<SearchItem>> handleSearch(@RequestBody SearchRequest request) {
		Page<Project> page;
		if (request.isGlobal()) {
			page = projectService.findAllProjects((String) request.getSearch().get(0).getValue(), request.getPage(),
					request.getPageSize(), request.getSort());
		} else {
			page = projectService.findProjectsForUser((String) request.getSearch().get(0).getValue(), request.getPage(),
					request.getPageSize(), request.getSort());
		}
	...
	public ResponseEntity<AntTableResponse<SearchItem>> searchSamples(@RequestBody SearchRequest request) {
		...
		if (request.isGlobal()) {
			samplePage = sampleService.searchAllSamples((String) request.getSearch().get(0).getValue(),
					request.getPage(), request.getPageSize(), newSort);
		} else {
			samplePage = sampleService.searchSamplesForUser((String) request.getSearch().get(0).getValue(),
					request.getPage(), request.getPageSize(), newSort);
		}
		...
		List<SearchItem> samples = samplePage.getContent().stream().map(join -> {
			...
			List<SearchProject> searchProjects = projects.stream()
					.map(projectSampleJoin -> new SearchProject(projectSampleJoin.getSubject(), 0L))
