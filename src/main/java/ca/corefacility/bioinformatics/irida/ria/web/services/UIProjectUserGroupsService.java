...
public class UIProjectUserGroupsService {
    ...
    public UIProjectUserGroupsService(ProjectService projectService, UserGroupService userGroupService, MessageSource messageSource) {
    ...
    public TableResponse<ProjectUserGroupsTableModel> getUserGroupsForProject(Long projectId, TableRequest request) {
        ...
        Page<UserGroupProjectJoin> userGroupJoins = userGroupService.getUserGroupsForProject(request.getSearch(), project, request.getCurrent(), request.getPageSize(), request.getSort());
        List<ProjectUserGroupsTableModel> groups = userGroupJoins.getContent().stream().map(j -> new ProjectUserGroupsTableModel(j.getObject(), j.getProjectRole().toString(), j.getMetadataRole().toString(), j.getCreatedDate())).collect(Collectors.toList());
    ...
    public String removeUserGroupFromProject(long projectId, long groupId, Locale locale) {
        ...
        try {
            ...
            return messageSource.getMessage("server.usergroups.remove.success", new Object[]{group.getLabel()}, locale);
        } catch (ProjectWithoutOwnerException e) {
            return messageSource.getMessage("server.usergroups.remove.error", new Object[]{group.getLabel()}, locale);
        }
    }
     ...
     */
    public String updateUserGroupRoleOnProject(Long projectId, Long groupId, String role, Locale locale) throws UIProjectWithoutOwnerException, UIConstraintViolationException {
        Project project = projectService.read(projectId);
        UserGroup group = userGroupService.read(groupId);
        ...
        try {
            ...
            return messageSource.getMessage("server.update.projectRole.success", new Object[]{group.getLabel(), roleString}, locale);
        } catch (ProjectWithoutOwnerException e) {
            throw new UIProjectWithoutOwnerException(messageSource.getMessage("server.ProjectRoleSelect.error", new Object[]{group.getLabel(), roleString}, locale));
            ...
            if (e.getRootCause() instanceof ConstraintViolationException) {
                ...
                for (ConstraintViolation constraintViolation : constraintViolationSet) {
                    ...
                    constraintViolationMessage += messageSource.getMessage(constraintViolation.getMessage(), new Object[]{}, locale) + "\n";
                }
            }
     ...
     */
    public String updateUserGroupMetadataRoleOnProject(Long projectId, Long groupId, String metadataRole, Locale locale) throws Exception {
        Project project = projectService.read(projectId);
        UserGroup group = userGroupService.read(groupId);
        ProjectMetadataRole projectMetadataRole;
        ...
        try {
            ...
            return messageSource.getMessage("server.update.metadataRole.success", new Object[]{group.getLabel(), roleString}, locale);
        } catch (Exception e) {
