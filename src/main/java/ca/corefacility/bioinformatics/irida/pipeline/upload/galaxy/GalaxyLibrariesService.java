...
public class GalaxyLibrariesService {
	...
	private static final Logger logger = LoggerFactory.getLogger(GalaxyLibrariesService.class);
	 ...
	 */
	private static List<String> LIBRARY_FAIL_STATES = Lists.newArrayList("paused", "error", "failed_metadata",
			"discarded");
	 ...
	 * @param dataStorage The {@link DataStorage} method to apply to this dataset.
	 * @return A dataset id for the dataset in this library.
	 * @throws UploadException If there was an issue uploading the file to the library.
	 */
	public String fileToLibrary(Path path, InputFileType fileType, Library library, DataStorage dataStorage)
	 ...
	 * @return An @{link Map} of paths and ids for each dataset object in this library.
	 * @throws UploadException If there was an issue uploading the file to Galaxy.
	 */
	public Map<Path, String> filesToLibraryWait(Set<Path> paths, Library library, DataStorage dataStorage)
			throws UploadException {
		...
		try {
				...
				public Void call() throws Exception {
					...
					for (Path path : paths) {
						...
						while (!LIBRARY_OK_STATE.equals(libraryDataset.getState())) {
							logger.trace("Waiting for library dataset " + libraryDataset.getId()
			...
			throw new UploadTimeoutException(
					"Timeout while uploading, time limit = " + libraryUploadTimeout + " seconds", e);
	...
	private InputFileType getFileType(Path path) throws IOException {
		...
		if (path.toString().startsWith(assemblyFileBaseDirectory.toString())) {
			return InputFileType.FASTA;
		} else {
