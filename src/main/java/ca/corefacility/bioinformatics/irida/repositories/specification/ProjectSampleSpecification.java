...
/**
 * Specification for searching and filtering {@link ProjectSampleJoin} or {@link ProjectSampleJoinMinimal} properties
 */
public class ProjectSampleSpecification {
	 ...
	 * Search a {@link Project} and it's associated {@link Project}s for {@link Sample}s based on filtering criteria.
	 *
	 * @param projectIds  {@link Long} of {@link Project} the {@link Sample}s must be found within.
	 * @param sampleNames {@link List} of {@link String} of Sample names to search
	 * @param sampleName  A single {@link Sample} name to loosely search
	 * @param searchTerm  {@link String} search term to search for.
	 * @param organism    {@link String} organism to search for.
	 * @param minDate     {@link Date} minimum date the sample was modified.
	 * @param maxDate     {@link Date} maximum date the sample was modified.
	 * @return {@link Specification} of {@link ProjectSampleJoinMinimal} for criteria to search based on the filtered
	 *         criteria.
	 */
	public static Specification<ProjectSampleJoinMinimal> getSamplesMinimal(List<Long> projectIds,
			List<String> sampleNames, String sampleName, String searchTerm, String organism, Date minDate,
			Date maxDate) {
		return (root, criteriaQuery, criteriaBuilder) -> {
			List<Predicate> predicates = new ArrayList<>();

			// Make sure the project is in the list of project requested;
			Expression<Long> projectExpression = root.get("project").get("id");
			predicates.add(projectExpression.in(projectIds));
