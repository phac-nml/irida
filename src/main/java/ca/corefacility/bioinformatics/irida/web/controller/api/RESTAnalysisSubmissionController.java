...
public class RESTAnalysisSubmissionController extends RESTGenericController<AnalysisSubmission> {
	...
	@RequestMapping("/{identifier}/samples")
	public ModelMap getSamplesForSubmission(@PathVariable Long identifier) {
		ModelMap model = new ModelMap();
		AnalysisSubmission submission = analysisSubmissionService.read(identifier);
		Set<SequenceFilePair> pairedInputFiles = submission.getPairedInputFiles();
		Set<SequenceFile> singleInputFiles = submission.getSingleInputFiles();

		Set<Sample> samples = new HashSet<>();

		for (SequenceFilePair pair : pairedInputFiles) {
			Join<Sample, SequenceFile> sampleForSequeneFile = sampleService.getSampleForSequeneFile(pair
					.getForwardSequenceFile());
			samples.add(sampleForSequeneFile.getSubject());
		}
		...
		for (SequenceFile file : singleInputFiles) {
			...
			Sample sample = sampleForSequeneFile.getSubject();

			samples.add(sample);
		}

		ResourceCollection<Sample> sampleResources = new ResourceCollection<>();
		samples.forEach((s) -> sampleResources.add(s));

		sampleResources
				.add(linkTo(methodOn(RESTAnalysisSubmissionController.class).getSamplesForSubmission(identifier))
						.withRel("self"));

		model.addAttribute(RESOURCE_NAME, sampleResources);
	...
	protected Collection<Link> constructCustomResourceLinks(AnalysisSubmission resource) {
		...
		resource.add(linkTo(methodOn(RESTAnalysisSubmissionController.class).getSamplesForSubmission(resource.getId()))
				.withRel(SAMPLES_REL));
