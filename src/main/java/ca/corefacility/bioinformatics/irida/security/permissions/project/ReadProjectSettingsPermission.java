...
/**
 * ISS Confirms that the authenticated user is allowed to read a project.
 */
@Component
public class ReadProjectSettingsPermission extends BasePermission<Project, Long> {

	private static final Logger logger = LoggerFactory.getLogger(ReadProjectSettingsPermission.class);
	public static final String PERMISSION_PROVIDED = "canReadProjectSettings";
	 ...
	 */
	@Autowired
	public ReadProjectSettingsPermission(final ProjectRepository projectRepository, final UserRepository userRepository,
			final ProjectUserJoinRepository pujRepository, final UserGroupProjectJoinRepository ugpjRepository,
			final UserGroupJoinRepository ugRepository, ProjectService projectService) {
	...
	public boolean customPermissionAllowed(final Authentication authentication, final Project p) {
		...
		for (final Join<Project, User> projectUser : projectUsers) {
			if (projectUser.getObject().equals(u) && (projectService.userHasProjectRole(u, p, ProjectRole.PROJECT_OWNER) || projectService.userHasProjectRole(u, p, ProjectRole.PROJECT_USER))) {
		...
		for (final UserGroupProjectJoin group : groups) {
			...
			if (inGroup && (projectService.userHasProjectRole(u, p, ProjectRole.PROJECT_OWNER) || projectService.userHasProjectRole(u, p, ProjectRole.PROJECT_USER))) {
				logger.trace("Permission GRANTED for [" + authentication + "] on project [" + p
