...
public class UserServiceImpl extends CRUDServiceImpl<Long, User> implements UserService {
	 ...
	 * @param projectSubscriptionRepository  the project subscription repository.
	...
	public UserServiceImpl(UserRepository userRepository, ProjectUserJoinRepository pujRepository,
			...
			AnnouncementUserJoinRepository announcementUserJoinRepository, PasswordEncoder passwordEncoder,
			Validator validator) {
	...
	public User create(@Valid User u) {
		...
		try {
			...
			for (Announcement announcement : announcements) {
				if (DateUtils.addMonths(announcement.getCreatedDate(), MARK_ANNOUNCEMENTS_READ_MONTHS)
						.before(now)) {
	...
	public User updateFields(Long uid, Map<String, Object> properties)
		...
		if (properties.containsKey(PASSWORD_PROPERTY)) {
			String password = properties.get(PASSWORD_PROPERTY)
					.toString();
	...
	public ProjectSubscription createProjectSubscription(User user, Project project) {
		return projectSubscriptionRepository.save(new ProjectSubscription(user, project));
	}
