...
function PipelineController(
  ...
  vm.launchButtonTitle = function () {
    if (this.shouldDisableLaunch()) {
      return window.__i18n("workflow.launch.btn.title-disarmed")
    } else {
      return window.__i18n("workflow.launch.btn.title");
    }
  };
    ...
    var // reference file id (use the most recently uploaded or the selected one)
      ref =
        typeof vm.uploadedReferenceFile !== "undefined"
          ...
          : Number(angular.element("option:selected").val()),
      ...
      description = angular.element("#analysis-description").val(),
...
function ParameterService() {
  ...
  if (window.PAGE.pipeline.parameters.length === 0) {
...
function DynamicSourceService() {
  ...
  if (window.PAGE.pipeline.dynamicSources == null) {
    window.PAGE.pipeline.dynamicSources = [
      {
  ...
  if (window.PAGE.pipeline.dynamicSources != null) {
    ...
    for (var i = 0; i < window.PAGE.pipeline.dynamicSources.length; i++) {
      settings.availableSettings[window.PAGE.pipeline.dynamicSources[i].id] =
        window.PAGE.pipeline.dynamicSources[i];
    }
  }
...
function ParametersWithChoicesService() {
  const svc = this;
  const params = {};
  if (window.PAGE.pipeline.paramsWithChoices != null) {
    for (let p of window.PAGE.pipeline.paramsWithChoices) {
      params[p.name] = {
        label: p.label,
        name: p.name,
