...
function NcbiBioSampleFiles({bioSample}: NcbiBioSampleFileProps): JSX.Element {
    return (
        <>
            <BasicList
            ...
            {bioSample.files.pairs.length > 0 &&
              <List header={<Typography.Text strong>Sequencing Files</Typography.Text>}
                    dataSource={bioSample.files.pairs} renderItem={item => (
                  <List.Item><BioSampleFile bioSampleFile={item}/></List.Item>)}/>}
        </>
    );
}
