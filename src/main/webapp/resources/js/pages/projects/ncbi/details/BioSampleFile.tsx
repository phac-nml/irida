...
function PairedEndLayout ({file} : {file: PairedEndSequenceFile}):JSX.Element {
    return  <div
        style={{
            display: "flex",
    ...
    >
        <Space><Avatar style={{backgroundColor: blue6}} icon={<SwapOutlined/>}/>
            {file.name}</Space>
        <Typography.Text type="secondary">{formatInternationalizedDateTime(file.createdDate)}</Typography.Text>
    </div>
}
