...
export async function getProjectReferenceFiles(projectId) {
  return await axios.get(setBaseUrl(`/projects/${projectId}/settings/ajax/reference/all`)
    ).then(({ data }) => data
    ).catch((error) => {
...
export async function downloadProjectReferenceFile(fileId) {
  window.open(
    setBaseUrl(`referenceFiles/download/${fileId}`),
    "_blank"
  );
}
...
export async function removeProjectReferenceFile(projectId, fileId) {
  return await axios.delete(setBaseUrl(`ajax/referenceFiles/delete/?fileId=${fileId}&projectId=${projectId}`)
