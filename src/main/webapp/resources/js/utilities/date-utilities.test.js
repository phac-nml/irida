import {
  ...
  fromNow, getDurationFromSeconds,
  isDate
} from "./date-utilities";

const MILLISECOND_DATE_1 = 1590093988318;
const STRING_DATE_1 ="May 21, 2020, 3:46:28 PM";
...
test("Should format to internationalized date and time", () => {
  expect(formatInternationalizedDateTime(MILLISECOND_DATE_1)).toBe(STRING_DATE_1);
})

test("Should format a date as instructed", () => {
  expect(formatDate({date: MILLISECOND_DATE_1})).toBe(STRING_DATE_1);
  expect(formatDate({date: MILLISECOND_DATE_1, format: { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }})).toBe("Thursday, May 21, 2020");
