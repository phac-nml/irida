...
public class UIActivitiesServiceTest {
	...
	public void testGetAllProjectsRecentActivity() {
		Mockito.when(eventService.getAllProjectsEvents(any()))
				.thenReturn(createMockProjectEvents(user2, null, true));
		...
		assertEquals("project_sample_added", ((Activity)response.getContent().get(8)).getType(), "Nineth should be a project_sample_added event");
	}
	...
	private Page<ProjectEvent> createMockProjectEvents(User currUser, Project specificProject, boolean adminListingPage) {
		...
		return new Page<ProjectEvent>() {
			List<ProjectEvent> eventListAll = ImmutableList.of(sampleAddedProjectEvent, userRoleSetProjectEventUser, userRemovedProjectEvent, userGroupRoleSetProjectEvent,
					sampleAddedProjectEvent2, userRoleSetProjectEventUser2, userRemovedProjectEvent2, userGroupRoleSetProjectEvent2, sampleAddedProjectEvent3);

			List<ProjectEvent> eventListUser = ImmutableList.of(sampleAddedProjectEvent, userRoleSetProjectEventUser, userRemovedProjectEvent, userGroupRoleSetProjectEvent);

			List<ProjectEvent> eventListUser2 = ImmutableList.of(sampleAddedProjectEvent2, userRoleSetProjectEventUser2, userRemovedProjectEvent2, userGroupRoleSetProjectEvent2, sampleAddedProjectEvent3);

			List<ProjectEvent> project1Events = ImmutableList.of(sampleAddedProjectEvent, userRoleSetProjectEventUser, userRemovedProjectEvent, userGroupRoleSetProjectEvent);

			List<ProjectEvent> project2Events = ImmutableList.of(sampleAddedProjectEvent2, userRoleSetProjectEventUser2, userRemovedProjectEvent2, userGroupRoleSetProjectEvent2, sampleAddedProjectEvent3);
			...
			public long getTotalElements() {
				if(currUser != null && currUser.getSystemRole() == Role.ROLE_ADMIN && adminListingPage) return eventListAll.size();
				else if(currUser == null && specificProject == project) return project1Events.size();
				else if(currUser == null && specificProject == project2) return project2Events.size();
				else if(currUser == user) return eventListUser.size();
				else return eventListUser2.size();
			}
			...
			public List<ProjectEvent> getContent() {
				if(currUser != null && currUser.getSystemRole() == Role.ROLE_ADMIN && adminListingPage) return eventListAll;
				else if(currUser == null && specificProject == project) return project1Events;
				else if(currUser == null && specificProject == project2) return project2Events;
				else if(currUser == user) return eventListUser;
				else return eventListUser2;
			}
