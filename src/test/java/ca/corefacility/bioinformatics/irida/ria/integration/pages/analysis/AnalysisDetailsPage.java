...
public class AnalysisDetailsPage extends AbstractPage {
	...
	/**
	 * Determines if tree shape tools aren't displayed on tree preview page
	 *
	 * @return {@link Boolean}
	 */
	...
	public String getCurrentTreeShapeTitleAttr() {
		...
		WebElement menuItem = menu
				.findElement(By.cssSelector(".ant-dropdown-menu-item-disabled .ant-dropdown-menu-title-content"));
	...
	public void updateTreeShape(String shape) {
		...
		for (WebElement button : buttons) {
			if (button.findElement(By.cssSelector("[role='img']")).getAttribute("data-shape").equals(shape)) {
				button.click();
				wait.until(ExpectedConditions.attributeContains(treeShapeTrigger.findElement(By.tagName("span")),
						"data-shape", shape));
				return;
			}
		}
	...
	public void selectedMetadataTemplate(String templateName) {
		...
		List<WebElement> templates = wait
				.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.className("t-template-field")));
	...
	public void toggleAllMetadataFields() {
		...
		wait.until(ExpectedConditions.attributeContains(switchElement, "aria-checked",
				toggleValue.equals("true") ? "false" : "true"));
	}
