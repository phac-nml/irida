...
function populateResultContent(html, item) {
    ...
    html = injectContent(html, findQueryInContent(q, item.content), '##Snippet##');
    return html;
}

function findQueryInContent(query, content) {
     // try to find a snippet of content where we found
    // the query string
    var index = content.indexOf(query);
    var snippet = content.substring(index - 50, index + 50);
    return snippet.replace(query, '<b>' + query + '</b>');
}
