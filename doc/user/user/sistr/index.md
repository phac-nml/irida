...
* **SISTR Information**
  ...
  * **Quality Control Details**: In the case of a status of `FAIL` the reasons why SISTR failed these results.
* **Serovar Predictions**
  * **Serovar (overall)**: The overall serovar prediction from all methods employed by SISTR.
  ...
  * **H1**: The H1 antigen.
  * **H2**: The H2 antigen.
  * **O antigen**: The O antigen.
* **cgMLST330**
  * **cgMLST subspecies**: The subspecies identified by comparing the genome to the cgMLST database in SISTR.
  * **cgMLST matching alleles**: The number of cgMLST alleles that were properly identifed by SISTR out of the total in the schema (330).
  * **cgMLST genome match**: The name of the closest matching genome in the SISTR database.
  * **cgMLST ST**: A SISTR Sequence Type number associated with the particular cgMLST profile.
* **Mash**
  * **Mash subspecies**: The subspecies identified by comparing the genome to the SISTR database using [Mash][].
